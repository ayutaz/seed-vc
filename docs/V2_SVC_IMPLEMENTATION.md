...
5. [訓練](#訓練)
   - [データ要件](#データ要件)
   - [クロスリンガルSVC向けデータセット構成例](#クロスリンガルsvc向けデータセット構成例)
   - [データ準備パイプライン](#データ準備パイプライン)
6. [テスト](#テスト)
...
| 推奨データ量 | 60〜70時間以上 |
| 最小話者数 | 20人以上 |
| アノテーション | **不要**（歌詞、F0、話者ラベル等すべて不要） |

#### 訓練対象と凍結コンポーネント

データセット設計にあたり、訓練時に何が学習されるかを理解することが重要です。

| コンポーネント | 訓練時 | 役割 |
|---|---|---|
| ASTRAL量子化 (content_extractor) | **凍結** | 言語コンテンツ抽出（話者非依存） |
| CAMPPlus (style_encoder) | **凍結** | 参照音声からスタイルベクトル抽出 |
| BigVGAN (vocoder) | **凍結** | mel → 波形変換 |
| RMVPE (F0抽出器) | **凍結** | F0抽出（訓練時オンライン実行） |
| **CFM + cfm_length_regulator** | **学習対象** | コンテンツ+F0+スタイル → mel再構成 |
| **AR + ar_length_regulator** | **学習対象(任意)** | narrow→wideトークン予測 |

CFMが「コンテンツトークン + F0 + スタイルベクトル → メルスペクトログラム」の変換を学ぶことが訓練の本質です。したがって、データセットには**F0のダイナミックレンジが大きい歌声データ**が最も効果的です。

#### クロスリンガルSVC向けデータセット構成例

英語ソース → 日本語話者の声で歌声変換を行う場合の推奨構成:

```
dataset_dir/
  ├── en_vocals/        ← 英語歌声（50〜60%）
  │   ├── singer01_song01_clip01.wav
  │   ├── singer01_song01_clip02.wav
  │   └── ...
  └── ja_vocals/        ← 日本語歌声（40〜50%）
      ├── singer20_song01_clip01.wav
      └── ...
```

| 言語 | 比率 | 目安 | 理由 |
|------|------|------|------|
| 英語歌声 | 50〜60% | 30〜40時間 | ソース言語。英語特有の音素（th, r/l, vなど）の再構成能力が最重要 |
| 日本語歌声 | 40〜50% | 25〜30時間 | ターゲット話者の言語。日本語話者の声質再現に必要 |

> **話し声データは不要です。** 歌声データだけで十分な量（60時間以上）と話者数（20人以上）が確保できれば、話し声を混ぜる必要はありません。
...
YouTube等の歌声動画
  ...
  ├─ yt-dlp で音声ダウンロード
  │
  ├─ Demucs (htdemucs_ft) でボーカル分離
  │     ※ ボーカル分離の品質が最終的なモデル品質に直結
  │
  ├─ 無音区間除去 + 30秒以下に分割
  │
  └─ dataset_dir/ に配置
...
| 注意点 | 説明 |
|--------|------|
| **伴奏残りの除去** | Demucsの`htdemucs_ft`モデルを使用し、分離後に伴奏残りが多いファイルは除外する |
| **スタジオ音源を優先** | ライブ音源よりスタジオ録音の方がきれいに分離できる |
| **メインボーカルのみ** | コーラス・ハモリは避け、ソロボーカルが望ましい |
| **話者の多様性** | 単一話者のデータだけでは推論時に別話者のスタイルベクトルに対応できない |
